      ...
      changeEmail: {
        ...
        sendChangeEmailVerification: async (
          ...
          const from =
            ...
            "ModernStack SaaS <no-reply@yourdomain.com>";
          if (!resendApiKey) {
            console.error(
              "RESEND_API_KEY not set. Unable to send email change verification.",
            );
            return;
          }
          try {
...
<p>If you didn't request this change, please ignore this email or contact support.</p>`,
    ...
    emailAndPassword: {
      ...
      sendResetPassword: async ({ user, url, token }, _request) => {
        ...
        if (!resendApiKey) {
          console.error(
            "RESEND_API_KEY not set. Unable to send reset password email.",
          );
          return;
        }
...
<p>If you didn't request this, you can safely ignore this email.</p>`,
          ...
          if (!res.ok) {
            const text = await res.text();
            console.error(
              "Resend API error sending reset email:",
              res.status,
              text,
            );
          }
