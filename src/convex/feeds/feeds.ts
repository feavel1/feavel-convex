...
export const unifiedFeedQuery = query({
	...
	handler: async (ctx, args) => {
		const {
			feedIds: originalFeedIds, // Renamed to avoid conflict
			type,
			language,
			publicOnly,
			slug,
		...
		const isSingleRequest = !!slug || originalFeedIds?.length === 1;
		...
		if (slug) {
			...
			if (feed.public) {
				// Add like data for single feed
				const feedLikes = await ctx.db
					.query('feedLikes')
				...
				if (user && user._id) {
					const userLike = await ctx.db
						.query('feedLikes')
						.withIndex('by_feed_and_user', (q) => q.eq('feedId', feed._id).eq('userId', user._id))
						.unique();
					isLiked = !!userLike;
				}

				return {
					feeds: [{ ...feed, likeCount: feedLikes.length, isLiked }],
					nextCursor: undefined
				};
			}

			// Private feed handling
			if (publicOnly) throw new Error('You do not have permission to access this feed');

			const user = await authComponent.getAuthUser(ctx).catch(() => null);
			if (!user) throw new Error('You do not have permission to access this feed');

			// Direct permission check (creator or collaborator)
			const isCreator = feed.createdBy === user._id;
			const isCollaborator = await ctx.db
				.query('feedCollaborators')
				.withIndex('feedId_userId', (q) => q.eq('feedId', feed._id).eq('userId', user._id))
				.unique();
			...
			// Add like data for private feed
			const feedLikes = await ctx.db
				.query('feedLikes')
				.withIndex('by_feed', (q) => q.eq('feedId', feed._id))
				.collect();
		...
		// 4. AUTHENTICATED USER MODE (non-public) OR USER-SPECIFIC QUERY
			...
			if (originalFeedIds?.length) {
				const validFeeds: FeedDoc[] = [];
				const feedChecks = originalFeedIds.map(async (feedId) => {
			...
			// 4b. USER-SPECIFIC FEEDS (created by specific user) OR USER'S PERSONAL FEEDS (created + collaborations)
				...
				if (userId) {
					...
					if (type) {
						creatorQuery = creatorQuery.filter((q) => q.eq(q.field('type'), type));
					}

					if (language) {
						creatorQuery = creatorQuery.filter((q) => q.eq(q.field('language'), language));
					}
				...
				// If no userId specified, fetch current user's feeds (created + collaborations)
					...
					const collaborationFeeds = (
						await Promise.all(collaborationFeedIds.map((id) => ctx.db.get(id as Id<'feed'>)))
					).filter(
		...
		if (isPublicQuery && !userId) {
			...
			if (type && language) {
				...
				const allPublicFeeds = await ctx.db
					...
					.withIndex('public_created_at', (q) => q.eq('public', true))
					.collect();
				totalCount = allPublicFeeds.filter((feed) => feed.language === language).length;
			} else {
				totalCount = (
					await ctx.db
						.query('feed')
		...
		// For user's personal feeds (creator + collaborations) or specific feed IDs
		else if (!originalFeedIds?.length) {
			// Count creator feeds
			...
			if (language) {
				creatorCountQuery = creatorCountQuery.filter((q) => q.eq(q.field('language'), language));
			}
			...
			const filteredCreatorFeeds = (
				...
				(feed) => (!type || feed.type === type) && (!language || feed.language === language)
			);
			...
			const allFeedIds = new Set([
				...collaborationFeeds
					.filter(
						(f): f is FeedDoc =>
							f !== null && (!type || f.type === type) && (!language || f.language === language)
					)
					.map((f) => f._id),
		...
		// Fetch like data for all returned feeds efficiently using parallel queries
		const feedIdsToProcess = finalFeeds.map((feed) => feed._id);

		// Use Promise.all to fetch like data for all feeds in parallel
		const feedLikeCountPromises = feedIdsToProcess.map((feedId) =>
			ctx.db
				.query('feedLikes')
				.withIndex('by_feed', (q) => q.eq('feedId', feedId))
				.collect()
				.then((likes) => ({ feedId, likes }))
		);

		const allFeedLikesResult = await Promise.all(feedLikeCountPromises);

		// Build map of feedId -> likes array
		const feedLikesMap = new Map<Id<'feed'>, Doc<'feedLikes'>[]>();
		allFeedLikesResult.forEach(({ feedId, likes }) => {
			feedLikesMap.set(feedId, likes);
		});

		// If user is authenticated, fetch their specific likes in parallel
		let userFeedLikesSet = new Set<Id<'feed'>>();
		if (user && user._id) {
			const userLikePromises = feedIdsToProcess.map((feedId) =>
				ctx.db
					.query('feedLikes')
					.withIndex('by_feed_and_user', (q) => q.eq('feedId', feedId).eq('userId', user._id))
					.unique()
					.then((like) => ({ feedId, liked: !!like }))
			);

			const userLikeResults = await Promise.all(userLikePromises);
			userLikeResults.forEach(({ feedId, liked }) => {
				if (liked) {
					userFeedLikesSet.add(feedId);
				}
			});
		}

		// Create the unified feed data structure combining feeds with their like data
		const unifiedFeeds = finalFeeds.map((feed) => ({
			...feed,
			likeCount: feedLikesMap.get(feed._id)?.length || 0,
			isLiked: userFeedLikesSet.has(feed._id)
		}));

		return {
			feeds: isSingleRequest ? unifiedFeeds.slice(0, 1) : unifiedFeeds,
