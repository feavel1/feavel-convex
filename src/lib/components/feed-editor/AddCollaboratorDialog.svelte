<script lang="ts">
	...
	async function handleAddCollaborator(selectedUserId: string) {
		...
		isAdding = true;
		try {
			await convexClient.mutation(api.feeds.feedCollaborators.addCollaborator, {
				feedId: feed._id,
		...
		<div class="grid gap-4 py-4">
			...
			{#if searchResults.length > 0}
				<div class="rounded-md border">
					<div class="border-b p-2 text-sm font-medium">Search Results</div>
					{#each searchResults as user (user.id)}
						<div
							class="flex cursor-pointer items-center justify-between rounded-md p-2 hover:bg-accent"
							role="button"
							tabindex="0"
			...
			<div class="grid gap-2">
				...
				<Select.Root type="single" bind:value={selectedRole}>
					...
					<Select.Content>
						<Select.Item value="read" label="Read access">Read - Can view the feed</Select.Item>
