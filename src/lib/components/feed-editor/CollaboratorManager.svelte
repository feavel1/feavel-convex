<script lang="ts">
	...
	let collaboratorsQuery = useQuery(api.feeds.feedCollaborators.getFeedCollaborators, () => ({
		feedId: feed._id
	}));
	...
	async function handleRemoveCollaborator() {
		...
		try {
			await convexClient.mutation(api.feeds.feedCollaborators.removeCollaborator, {
				feedId: feed._id,
	...
	async function handleUpdateRole(collaboratorId: string, newRole: 'read' | 'edit' | 'admin') {
		...
		try {
			await convexClient.mutation(api.feeds.feedCollaborators.updateCollaboratorRole, {
				feedId: feed._id,
...
<div class="mt-6 border-t pt-6">
	<div class="mb-4 flex items-center justify-between">
		...
		{#if isOwner}
			<Button variant="outline" onclick={() => (showAddCollaboratorDialog = true)}>
	...
	{:else if error}
		<div class="text-sm text-red-500">Error loading collaborators: {error.message}</div>
						...
						{#each collaborators as collaborator (collaborator._id)}
							<Table.Row>
								...
								<Table.Cell>
									...
									{:else}
										<Badge
											variant={collaborator.role === 'admin'
												? 'default'
