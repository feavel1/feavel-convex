<script lang="ts">
  ...
  let {
    feed: feedProp,
    debouncedSave: debouncedSaveProp,
  } = $props();
  ...
  $effect(() => {
    if (feedProp?.type && feedProp.meta) {
      ...
      if (feedProp.type === 'article' && feedProp.meta) {
        ...
        if (typeof feedProp.meta.product?.price === 'number' || typeof feedProp.meta.product?.price === 'string') {
  ...
  const updateArticleMeta = () => {
    ...
    const keywordsArray = editorMeta.article.keywords.split(',').map(s => s.trim()).filter(s => s);
    const linksArray = editorMeta.article.links.split(',').map(s => s.trim()).filter(s => s);
  ...
  const updateServiceMeta = () => {
    ...
    const linksArray = editorMeta.service.links.split(',').map(s => s.trim()).filter(s => s);
  ...
  const updateProductMeta = () => {
    ...
    const linksArray = editorMeta.product.links.split(',').map(s => s.trim()).filter(s => s);
      ...
      <div class="space-y-2">
        <div class="flex items-center space-x-2">
          <Switch
            ...
            onclick={() => {feedProp.public=!feedProp.public; debouncedSaveProp()}}
          />
          <Label
            for="public-toggle"
