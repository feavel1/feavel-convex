<script lang="ts">
	...
	let { children, className = undefined }: { children?: any; className?: string } = $props();
</script>
...
>
	<div class="relative isolate z-0 flex w-full flex-1 scale-y-125 items-center justify-center animate-lamp-start">
		<div
			class="absolute inset-auto right-1/2 h-56 w-[15rem] animate-lamp-grow-1 overflow-visible text-white [--conic-position:from_70deg_at_center_top]"
		...
		<div
			class="absolute inset-auto left-1/2 h-56 w-[15rem] animate-lamp-grow-2 overflow-visible text-white [--conic-position:from_290deg_at_center_top]"
			style="background-image: conic-gradient(var(--conic-position), var(--tw-gradient-stops));"
		>
			<div
		...
		<div
			class="absolute inset-auto z-30 h-36 w-[8rem] -translate-y-[6rem] rounded-full bg-cyan-400 blur-2xl animate-lamp-grow-3"
		></div>

		<div
			class="absolute inset-auto z-50 h-0.5 w-[15rem] -translate-y-[7rem] bg-cyan-400 animate-lamp-grow-4"
		></div>
...
<style>
	...
	.animate-lamp-start .animate-lamp-grow-1 {
		animation: lamp-grow 0.8s ease-in-out 0.3s both;
	}

	.animate-lamp-start .animate-lamp-grow-2 {
		animation: lamp-grow 0.8s ease-in-out 0.3s both;
	}

	.animate-lamp-start .animate-lamp-grow-3 {
		animation: lamp-grow-3 0.8s ease-in-out 0.3s both;
	}

	.animate-lamp-start .animate-lamp-grow-4 {
		animation: lamp-grow-4 0.8s ease-in-out 0.3s both;
	}
</style>
