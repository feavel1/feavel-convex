...
<AnimatePresence show={true}>
	...
	>
		<div
			use:motion
			class={cn(
				'fixed inset-x-0 top-6 z-[10] mx-auto flex max-w-fit items-center justify-center space-x-4 rounded-full border border-transparent bg-gray-100 py-2 pr-2 pl-8  shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] dark:border-white/[0.2] dark:bg-black'
			)}
		>
			{#each navItems as navItem, idx (`link=${idx}`)}
				<Button variant="ghost" href={navItem.link} class={cn('')}>
					<svelte:component
						this={navItem.icon}
			...
			<Button
				href="/auth/sign-up"
				class="relative rounded-full border border-neutral-500 px-4 text-sm font-medium  dark:border-red-600 "
			>
				<span>Account</span>
				<!-- <span
